...
const MAP = "/levels/1/map1.json";
const TILESET = "/levels/1/tileset.png";
...
export default function Stage1(props) {
    ...
    async function uploadTime(newUsername, newTime, levelTimes) {
        ...
        const L1 = [];
        for (const timeL1 in props.levelTimes) {
            L1.push({
                time: props.levelTimes[timeL1].time,
                username: props.levelTimes[timeL1].username
            })
        }
        L1.sort(sortItems);
        ...
        for (const key in levelTimes) {
            if (levelTimes[key].username == newUsername) {
                ...
                if (levelTimes[key].time > newTime) {
                    try {
                        firebase.database().ref("levels/l1/" + key).update({
                            time: newTime
                        });
                        setEndState({ msg: "Awsome Run, Uploaded new Time succsesfully!", oldTime: oldTime, pb: newTime, wr: L1[0] });
                        break;
                    } catch (err) {
                        console.log(err);
                ...
                if (levelTimes[key].time < newTime) {
                    setEndState({ msg: "U didn't improve your time, try again!", oldTime: oldTime, pb: oldTime, wr: L1[0] });
                    break;
                }
                if (levelTimes[key].time == newTime) {
                    setEndState({ msg: "U didn't improve your time, try again!", oldTime: oldTime, pb: oldTime, wr: L1[0] });
            ...
            if(posNum >= objNum) {
                try {
                    firebase.database().ref("levels/l1/").push({
                        time: newTime,
                        username: newUsername
                    });
                    setEndState({ msg: "Awsome Run, Uploaded new Time succsesfully!", oldTime: "none", pb: newTime, wr: L1[0] });
                    break;
                } catch (err) {
                    console.log(err);
    ...
    if (props.userImg) {
        return (
            <Container pageName="Level 1" noscroll={`${finishGUI ? "noscroll" : ""}`}>
