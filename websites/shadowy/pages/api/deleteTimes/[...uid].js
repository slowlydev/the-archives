    ...
    if (req.method === "POST") {
        ...
        if (isAdmin) {
            if (req.query.uid[1] && req.query.uid[2] && req.query.uid[3]) {
                ...
                await adminApp().database().ref(`levels/${req.query.uid[2]}/${req.query.uid[3]}`).remove();

                return res.status(200).json({
                    msg: "Succsesfully deleted times of user!", uid: req.query.uid[1]
                }); 
            } else {
                return res.status(400).json({
                    msg: "Failed to delete times of user!", uid: req.query.uid[1]
                });
            }
