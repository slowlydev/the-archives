...
function askUserforPath(question) {
...
async function start() {
    ...
    const fileDataString = file.toString();
    ...
    const original = charAndCount.sort((a, b) => a.count - b.count);
    let charArray = original.map((charAndCountToChar) => charAndCountToChar.char);
    let charfreq = original.map((charAndCountToFreq) => charAndCountToFreq.count);
    let queue = [];
    let rainbowTable = [];
